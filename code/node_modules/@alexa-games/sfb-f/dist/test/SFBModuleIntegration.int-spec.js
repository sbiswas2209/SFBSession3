...
const SFBContentTester_1 = require("./SFBContentTester");
...
const SFBExtension_1 = require("../extensions/SFBExtension");
...
describe("Scene Transition Baseline", function () {
    ...
    before(async function () {
        let importer = new importer_1.SFBImporter(undefined, undefined, [
            new SFBExtension_1.GlobalDirectionsExtension([]),
            new SFBExtension_1.SnippetExtension({}),
            new SFBExtension_1.VoiceOverExtension("{{file_name}}")
        ]);
        ...
        contentTester = new SFBContentTester_1.SFBContentTester(importedStory, [
            new SFBExtension_1.GlobalDirectionsExtension([]),
            new SFBExtension_1.SnippetExtension({}),
            new SFBExtension_1.VoiceOverExtension("{{file_name}}")
        ], ffmpeg.path);
...
describe("Skill Flow Builder Test", function () {
    ...
    it("audio source tag", async function () {
        ...
        await contentTester.assertSpeech("playing audio now: <audio src='https://sfb-framework.s3.amazonaws.com/examples/sounds/Trumpet_1.mp3' />  Playing same tag, but with a snippet. <trumpets> Thanks for testing. Bye.");
        await contentTester.assertPrettySpeech("playing audio now: <audio src='https://sfb-framework.s3.amazonaws.com/examples/sounds/Trumpet_1.mp3' />  Playing same tag, but with a snippet. <trumpets> Thanks for testing. Bye.");
        await contentTester.assertPrettySpeechSSML("playing audio now: <audio src='https://sfb-framework.s3.amazonaws.com/examples/sounds/Trumpet_1.mp3' />  Playing same tag, but with a snippet. <trumpets> Thanks for testing. Bye.");
        await contentTester.assertSpeechSSMLPerScene("playing audio now: <audio src='https://sfb-framework.s3.amazonaws.com/examples/sounds/Trumpet_1.mp3' />  Playing same tag, but with a snippet. <trumpets> Thanks for testing. Bye.");
    });
    it("background music");
    // , async function () {
    //     await contentTester.givenStartInvoked();
...
describe("Snippet Test", function () {
    ...
    before(async function () {
        ...
        let importer = new importer_1.SFBImporter(undefined, undefined, [
            new SFBExtension_1.GlobalDirectionsExtension([]),
            new SFBExtension_1.SnippetExtension(snippetFile),
            new SFBExtension_1.VoiceOverExtension("{{file_name}}")
        ]);
        let importedStory = await importer.importABCStory(DEFAULT_IMPORT_PLUGIN_NAME, "", "snippets test", "snippets test", true, {
            customSlots: storyUtilities_1.loadTestResourceAsObject(storyUtilities_1.POSITIVE_TEST_STORY_DIRECTORY, "SlotTypes.json"),
...
//# sourceMappingURL=SFBModuleIntegration.int-spec.js.map